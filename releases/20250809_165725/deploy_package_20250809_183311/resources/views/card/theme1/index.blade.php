@php
    ...
    if (!is_null($custom_html)) {
        $customhtml->is_map_iframe_enabled == '1' ? ($is_map_iframe_enabled = true) : ($is_map_iframe_enabled = false);
    }
...
<body class="tech-card-body">
    <div class="{{ \App\Models\Utility::themeOne()['theme1'][$business->theme_color]['theme_name'] }}"
                        ...
                        @foreach ($card_theme->order as $order_key => $order_value)
                            @if ($j == $order_value)
                                ...
                                @if ($order_key == 'contact_info')
                                    ...
                                    @include('components.card-wallet-buttons')
                                @endif
                             ...
                             @if ($order_key == 'map_iframe' )
                                    <section id="map_iframe_preview" class="card-footer print map_iframe" style="background:white !important; padding: 1px 15px;@if(!$is_map_iframe_enabled)
                                                  display: none;
                                                @endif">
       ...
       var preview = @if(isset($map_iframe) && $map_iframe!="") true @else false @endif ;
        function initMap() {
            console.log("test", document.getElementById('map_iframe_preview_'))
                ...
                var autocompleteService = new google.maps.places.AutocompleteService()
                 ...
                 autocompleteService.getPlacePredictions({ input: address }, function(predictions, status) {
                    if (status == google.maps.places.PlacesServiceStatus.OK && predictions) {
                        ...
                        document.querySelector(".hide_afb").setAttribute("hidden","hidden")
                        ...
                        predictionsContainer.innerHTML = 'No predictions found';
                    }
                });
       ...
       function displayMapPreview(place) {
            const mapPreview = new google.maps.Map(document.getElementById('map_iframe_preview_'), {
                center: place.geometry.location,
                zoom: 15,
                disableDefaultUI: true // Disable default map type controls
            });
            
            let place_name="{!! substr(stripslashes($map_iframe), 0, strpos(stripslashes($map_iframe), ',')) !!}";
            ...
            const infowindow = new google.maps.InfoWindow({
                content: '<div style="padding: 8px 12px;color:black;margin-bottom:5px" ><strong style="color: #3d3936; font-family: Montserrat;">'+ (place_name!="" ? place_name:'') + '</strong><br>' +
                         '<span style="color: #907C6A; font-family: Montserrat;">' + place.formatted_address + '</span>' +
        ...
        $(document).ready(function() {
            ...
            function generateQRCode() {
                try {
                    ...
                    @if (isset($plan->enable_qr_code) && $plan->enable_qr_code == 'on')
                        ...
                        // Only add image if the element exists
                        var imageBuffer = $("#image-buffers")[0];
                        if (imageBuffer) {
                            qrOptions.image = imageBuffer;
                        }

                        $('.shareqrcode').qrcode(qrOptions);
                    @else
                        $('.shareqrcode').qrcode({
                            render: 'image',
                            size: 500,
                    ...
                    try {
                        ...
                        $('.shareqrcode').html('<p style="color: red;">QR Code generation failed</p>');
                    }
                }
            }
            ...
            function ensureQRCodeLibrary() {
                if (typeof $.fn.qrcode !== 'undefined') {
                    ...
                    setTimeout(ensureQRCodeLibrary, 200);
                }
            }
            
            setTimeout(ensureQRCodeLibrary, 100);
        });
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Auto download vCard if enabled
            @if(isset($auto_download_vcard) && $auto_download_vcard)
            setTimeout(function() {
                autoDownloadVCard('{{ $business->slug }}');
        ...
        // Function to automatically download vCard
        function autoDownloadVCard(slug) {
            // Try iframe approach first
            try {
                var iframe = document.createElement('iframe');
                iframe.style.display = 'none';
                iframe.src = '/download/' + slug;
                
                // Add error handling
                iframe.onerror = function() {
                    // Fallback to direct link approach
                    fallbackDownload(slug);
                };
        ...
        // Fallback function using direct link click
        function fallbackDownload(slug) {
            try {
                ...
                // If both methods fail, we can't do much more
            }
        }
    ...
    <script src="{{ asset('custom/js/tap-analytics.js') }}"></script>
    
    <!-- Read More Feature for Why Choose Magic African Safaris Section -->
    <style>
        /* Uniform height for feature cards */
        .why-choose-section .feature-card,
        .why-choose-section [class*='card'],
        .why-choose-section [class*='feature'],
         .feature-card,
         [class*='card'],
         [class*='feature'] {
            display: flex;
        ...
        /* Text container with uniform initial height */
        .read-more-container {
            position: relative;
            overflow: hidden;
        ...
        .read-more-container.collapsed {
            max-height: 120px; /* Uniform height for all cards */
        }
        ...
        /* Ensure cards maintain uniform height when collapsed */
        .why-choose-section .feature-card.collapsed-state,
         .feature-card.collapsed-state {
            height: auto;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Find the Why Choose Magic African Safaris section
            function findWhyChooseSection() {
                // Try multiple selectors to find the section
                const headings = document.querySelectorAll('h2, h3, .section-title h2');
                let targetSection = null;
                
                headings.forEach(heading => {
                    const text = heading.textContent || heading.innerText;
                    if (text.toLowerCase().includes('why choose') || 
                        text.toLowerCase().includes('our success')) {
            ...
            function initializeReadMore() {
                const section = findWhyChooseSection();
                if (!section) {
                    console.log('Why Choose section not found');
                    return;
                }
                
                // Add class to section for easier targeting
                section.classList.add('why-choose-section');
                
                // Find all text containers in the section
                // Look for common card/feature structures
                const cards = section.querySelectorAll('[class*=card], [class*=feature], .col-sm-6, .col-md-4, .col-lg-4, div[class*=item]');
                
                cards.forEach((card, index) => {
                    // Find text content - look for paragraphs or divs with text
                    const textElements = card.querySelectorAll('p, div:not([class*=icon]):not([class*=image]):not([class*=img])');
                    
                    textElements.forEach(textEl => {
                        const text = textEl.textContent.trim();
                        // Skip if text is too short or already has read more
                        if (text.length < 100 || textEl.closest('.read-more-wrapper')) {
                            return;
                        }
                        
                        // Check if this element is likely the main description
                        const parent = textEl.parentElement;
                        if (parent.querySelector('img') || parent.querySelector('[class*=icon]')) {
                            // This is likely a feature card
                            wrapTextWithReadMore(textEl, card);
                        }
                    });
                    
                    // If no text element was found, try to find description in the card
                    if (!card.querySelector('.read-more-wrapper')) {
                        const allText = card.textContent.trim();
                        const images = card.querySelectorAll('img, [class*=icon]');
                        if (images.length > 0 && allText.length > 150) {
                            // Find the main text paragraph
                            const paragraphs = card.querySelectorAll('p');
                            if (paragraphs.length > 0) {
                                wrapTextWithReadMore(paragraphs[paragraphs.length - 1], card);
                            }
                        }
                    }
                });
                
                // If no cards found with the above selectors, try a different approach
                if (section.querySelectorAll('.read-more-wrapper').length === 0) {
                    // Look for direct children that might be feature items
                    const directChildren = Array.from(section.children);
                    directChildren.forEach(child => {
                        const text = child.textContent.trim();
                        if (text.length > 150 && child.querySelector('p')) {
                            const paragraphs = child.querySelectorAll('p');
                            paragraphs.forEach(p => {
                                if (p.textContent.trim().length > 100) {
                                    wrapTextWithReadMore(p, child);
                                }
                            });
                        }
                    });
            ...
            function wrapTextWithReadMore(textElement, card) {
                // Skip if already wrapped
                if (textElement.closest('.read-more-wrapper')) {
                    return;
                }
                ...
                container.className = 'read-more-container collapsed';
                
                // Clone the text element to preserve formatting
                ...
                tempDiv.appendChild(clonedText.cloneNode(true));
                document.body.appendChild(tempDiv);
                
                const needsTruncation = tempDiv.scrollHeight > 120;
                ...
                // Ensure card maintains uniform height
            ...
            setTimeout(initializeReadMore, 500);
            setTimeout(initializeReadMore, 1000);
        });
    </script>
</body>
