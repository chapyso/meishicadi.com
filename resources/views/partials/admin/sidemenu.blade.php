        ...
        <ul class="dash-navbar">
            ...
            <li class="dash-item dash-hasmenu">
                <a class="dash-link {{ Request::segment(1) == 'new_business' || Request::segment(1) == 'business' ? 'active' : '' }}"
                    ...
                    aria-controls="navbar-getting-started"><span class="dash-micon"><i
                            class="ti ti-credit-card"></i></span><span class="dash-mtext">{{ __('Create Your Card') }}</span><span
                    ...
                    @if (\Auth::user()->can('create business'))
                        <li class="dash-item {{ Request::segment(1) == 'new_business' ? 'active' : '' }}">
                            <a href="#" class="dash-link" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                ...
                                data-bs-whatever="{{ __('Create New Business') }}"> {{ __('Create Card') }}
                            </a>
                        </li>
                    @endif
                    @if (\Auth::user()->can('manage business'))
                        <li class="dash-item {{ Request::segment(1) == 'business' ? 'active' : '' }}">
                            <a class="dash-link" href="{{ route('business.index') }}">{{ __('Manage Card') }}</a>
            ...
            <li class="dash-item dash-hasmenu">
                <a class="dash-link {{ Request::segment(1) == 'employee' || Request::segment(1) == 'client' ? 'active' : '' }}"
                    ...
                    aria-controls="navbar-getting-started"><span class="dash-micon"><i
                            class="ti ti-users"></i></span><span class="dash-mtext">{{ __('Manage Admins') }}</span><span
                        class="dash-arrow"><i data-feather="chevron-right"></i></span>
                </a>
                <ul class="dash-submenu">
                    @if (Gate::check('manage user'))
                        <li class="dash-item dash-hasmenu {{ Request::segment(1) == 'users' ? 'active open' : '' }}">
                            <a class="dash-link"
                                ...
                                href="{{ route('users.index') }}">{{ __('Admin') }}</span></a>
                        </li>
                    @endif
            ...
            @if($plan && $plan->enable_wallet == 'on')
                <li class="dash-item {{ Request::segment(1) == 'wallet' ? 'active' : '' }}">
                    <a href="{{ route('wallet.index') }}" class="dash-link">
                        <span class="dash-micon"><i class="fas fa-wallet"></i></span>
                        <span class="dash-mtext">{{ __('Wallet') }}</span>
                    </a>
                </li>
            @endif
            ...
            @php
                $user = \Auth::user();
                $isPlanExpired = $user->plan_expire_date && $user->plan_expire_date < date('Y-m-d');
            @endphp
            @if($isPlanExpired)
                <li class="dash-item {{ Request::route()->getName() == 'plan.expired' ? 'active' : '' }}">
                    <a href="{{ route('plan.expired') }}" class="dash-link">
                        <span class="dash-micon"><i class="ti ti-alert-triangle text-warning"></i></span>
                        <span class="dash-mtext">{{ __('Plan Expired') }}</span>
                    </a>
                </li>
            @endif
            @if (\Auth::user()->can('manage contact'))
                <li class="dash-item {{ Request::segment(1) == 'contacts' ? 'active' : '' }}">
                    <a href="{{ route('contacts.index') }}" class="dash-link"><span class="dash-micon"><i
                                class="ti ti-magnet"></i></span><span class="dash-mtext">{{ __('Leads') }}</span></a>

                </li>
        @if (\Auth::user()->can('manage plan'))
    <!-- Contact Support Button -->
    <li class="dash-item {{ \Request::route()->getName() == 'contact-support' ? 'active' : '' }}">
        <a href="https://chapysocial.com/support/" class="dash-link">
            <span class="dash-micon"><i class="ti ti-phone"></i></span>
            <span class="dash-mtext">{{ __('Contact Support') }}</span>
        </a>
    </li>
@endif
            ...
            @if (\Auth::user()->can('manage plan'))
              ...
              @if (\Auth::user()->can('manage plan'))
                <li class="dash-item {{ \Request::route()->getName() == 'plans' || \Request::route()->getName() == 'stripe' ? 'active' : '' }}">
                    <a href="https://wetransfer.meishicadi.com/" class="dash-link bulk-transfer-link" target="_blank" rel="noopener noreferrer">
                        <span class="dash-micon"><i class="ti ti-share"></i></span>
                        <span class="dash-mtext">{{ __('Bulk Transfer') }}</span>
                    </a>
                </li>
            @endif
            <!--@endif-->
...
<script>
// Remove any arrows from the Bulk Transfer link
document.addEventListener('DOMContentLoaded', function() {
    const bulkTransferLink = document.querySelector('.bulk-transfer-link');
    if (bulkTransferLink) {
        // Remove any ::after content
        ...
        // Also check for any text content that might contain arrows
        const mtext = bulkTransferLink.querySelector('.dash-mtext');
        if (mtext && mtext.textContent.includes('-->')) {
            mtext.textContent = mtext.textContent.replace(/-->/g, '');
        }
    }
});
</script>
