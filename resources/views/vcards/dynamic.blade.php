    ...
    <div class="container p-0 vcard22-main position-relative">
      ...
      <div class="main-content mx-auto w-100 overflow-hidden bg-green">
        ...
        <div class="profile-actions mt-4 text-center">
            ...
            <button id="add-to-home" class="btn btn-primary mb-2">
                <i class="fas fa-mobile-alt me-2"></i>Add to Home Screen
            </button>
            
            <!-- Download App -->
            <a href="https://play.google.com/store/apps/details?id=YOUR_APP_ID" class="btn btn-success mb-2" target="_blank">
                <i class="fab fa-google-play me-2"></i>Download App
            </a>
            
            <!-- Wallet Options (Premium Feature) -->
            <div id="wallet-options" class="d-none">
                <div class="dropdown d-inline-block mb-2">
                    <button class="btn btn-warning dropdown-toggle" type="button" id="walletDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="walletDropdown">
                        <li><a class="dropdown-item" href="#" id="add-to-apple-wallet">
                            <i class="fab fa-apple me-2"></i>Add to Apple Wallet
                        </a></li>
                        <li><a class="dropdown-item" href="#" id="add-to-google-wallet">
            ...
            <div id="wallet-upgrade-prompt" class="d-none">
                <div class="alert alert-warning mb-2">
                    <div class="d-flex align-items-center">
                        <i class="fas fa-crown me-2"></i>
                        <div class="flex-grow-1">
                            <strong>Premium Feature:</strong> Wallet integration requires a premium subscription.
                            <a href="/subscription/upgrade" class="btn btn-sm btn-warning ms-2">
            ...
            <!-- Non-owner message -->
            <div id="non-owner-message" class="alert alert-info d-none">
                <i class="fas fa-info-circle me-2"></i>
                <small>Only the card owner can add this vCard to their wallet</small>
            </div>
        </div>
        ...
        // Check wallet access and show appropriate buttons
        async function checkWalletAccess() {
            try {
                const vcardId = {{ $vcard->id ?? 'null' }};
                
                if (!vcardId) {
                    return;
                }
                
                const response = await fetch(`/wallet/access/${vcardId}`, {
                    method: 'GET',
                    headers: {
                ...
                if (result.success) {
                    if (result.can_add_to_wallet && result.has_wallet_feature) {
                        // Show wallet options for premium users
                        document.getElementById('wallet-options').classList.remove('d-none');
                    } else if (result.can_add_to_wallet && !result.has_wallet_feature) {
                        // Show upgrade prompt for non-premium users
                        document.getElementById('wallet-upgrade-prompt').classList.remove('d-none');
                    } else {
                        // Show message for non-owners
                        document.getElementById('non-owner-message').classList.remove('d-none');
                    }
                }
        ...
        document.getElementById('add-to-apple-wallet').addEventListener('click', async function(e) {
            e.preventDefault();
            const button = this;
            ...
            try {
                ...
                if (!vcardId) {
                    throw new Error('Vcard ID not found');
                }
                
                const response = await fetch(`/wallet/apple/${vcardId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                if (result.success) {
                    ...
                    button.innerHTML = '<i class="fas fa-check me-2"></i>Pass Generated!';
                    setTimeout(() => {
                    ...
                    if (confirm('This is a premium feature. Would you like to upgrade your subscription?')) {
                        window.location.href = '/subscription/upgrade';
                    }
                    button.innerHTML = originalText;
                    button.disabled = false;
                } else {
                    throw new Error(result.message || 'Failed to generate Apple Wallet pass');
                }
                
            } catch (error) {
                console.error('Apple Wallet Error:', error);
                button.innerHTML = '<i class="fas fa-exclamation-triangle me-2"></i>Error: ' + error.message;
                setTimeout(() => {
                    button.innerHTML = originalText;
                    button.disabled = false;
                }, 3000);
        ...
        document.getElementById('add-to-google-wallet').addEventListener('click', async function(e) {
            e.preventDefault();
            const button = this;
            const originalText = button.innerHTML;
            ...
            try {
                ...
                const response = await fetch(`/wallet/google/${vcardId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                if (result.success) {
                    ...
                    throw new Error(result.message || 'Failed to generate Google Wallet pass');
                }
        ...
        // Initialize wallet access check when page loads
