...
class AppleWalletService
{
    ...
    public function testConfiguration(): array
    {
        $results = [
            ...
            'team_identifier_set' => !empty($this->teamIdentifier),
            'pass_type_identifier_set' => !empty($this->passTypeIdentifier),
            'certificate_password_set' => !empty($this->certificatePassword),
        ];

        // Test certificate if it exists
        if ($results['certificate_exists']) {
            try {
                $certData = openssl_pkcs12_read(file_get_contents($this->certificatePath), $certs, $this->certificatePassword);
                $results['certificate_valid'] = $certData !== false;
                if ($certData) {
                    $results['certificate_info'] = openssl_x509_parse($certs['cert']);
                }
            } catch (\Exception $e) {
                $results['certificate_valid'] = false;
                $results['certificate_error'] = $e->getMessage();
            }
        }

        // Check if all required settings are configured
        $results['fully_configured'] = $results['enabled'] && 
                                     $results['certificate_exists'] && 
                                     $results['certificate_valid'] && 
                                     $results['team_identifier_set'] && 
                                     $results['pass_type_identifier_set'];
