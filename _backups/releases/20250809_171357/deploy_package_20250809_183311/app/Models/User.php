...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    /**
     * Check if this user can be impersonated by a super admin.
     *
     * @return bool
     */
    public function canBeImpersonated()
    {
        return $this->type !== 'super admin' && $this->id !== 1; // Prevent impersonating super admins and the main admin
    }
    ...
    /**
     * Check if user has access to premium features (super admin or has premium plan)
     */
    public function hasPremiumAccess()
    {
        return $this->isSuperAdmin() || ($this->plan && $this->plan > 1);
    }
    ...
    {
        \Log::info('User extendSubscription method called', [
            'user_id' => $this->id,
            'user_name' => $this->name,
            'months' => $months,
            'current_plan_expire_date' => $this->plan_expire_date
        ]);

        try {
            // If plan is lifetime, no need to extend
            if ($this->plan_expire_date === null) {
                \Log::warning('Cannot extend lifetime plan', ['user_id' => $this->id]);
                return [
                    'is_success' => false,
                    'error' => 'Cannot extend lifetime plan.'
                ];
            }

            // Calculate new expiration date
            $currentExpireDate = $this->plan_expire_date ? Carbon::parse($this->plan_expire_date) : Carbon::now();
            $newExpireDate = $currentExpireDate->addMonths($months);
            
            \Log::info('Calculated new expiry date', [
                'user_id' => $this->id,
                'current_expire_date' => $currentExpireDate->format('Y-m-d'),
                'new_expire_date' => $newExpireDate->format('Y-m-d'),
                'months_added' => $months
            ]);
            
            // Update the expiration date
            $this->plan_expire_date = $newExpireDate->format('Y-m-d');
            $this->save();

            \Log::info('Subscription extended successfully', [
                'user_id' => $this->id,
                'new_expire_date' => $this->plan_expire_date
            ]);
