...
use App\Mail\SubscriptionExtensionUser;
use App\Mail\SubscriptionExtensionAdmin;
...
class UserController extends Controller
{
    ...
    public function LoginWithCompany(Request $request, User $user, $id)
    {
        ...
        if (!auth()->check() || auth()->user()->type !== 'super admin') {
            return redirect()->back()->with('error', __('Permission denied. Only Super Admins can impersonate users.'));
        }

        $targetUser = User::find($id);
        
        // Validate target user exists and is not a super admin
        if (!$targetUser) {
            return redirect()->back()->with('error', __('User not found.'));
        }

        if ($targetUser->type === 'super admin') {
            return redirect()->back()->with('error', __('Cannot impersonate another Super Admin.'));
        }
        ...
        // Start impersonation
        Impersonate::take($request->user(), $targetUser);
        
        return redirect('/home')->with('success', __('Successfully logged in as ') . $targetUser->name);
    }
    ...
    public function leaveImpersonation(Request $request)
    {
        if ($request->user()->isImpersonating()) {
            ...
            \Log::info('Super Admin impersonation ended', [
                ...
                'target_user_name' => auth()->user()->name,
                'ip_address' => $request->ip(),
                'user_agent' => $request->userAgent()
            ]);

            Impersonate::leave($request->user());
            return redirect('/users')->with('success', __('Successfully returned to Super Admin account.'));
        }
        return redirect('/home');
    }

    /**
     * Show subscription extension modal
     */
    public function showSubscriptionExtension($user_id)
    {
        $user = User::find($user_id);
        if (!$user) {
        ...
        return view('user.subscription-extension', compact('user'));
    }

    /**
     * Extend user subscription
     */
    public function extendSubscription(Request $request, $user_id)
    {
        \Log::info('Extend subscription request received', [
            'user_id' => $user_id,
            'request_data' => $request->all(),
        ...
        $request->validate([
            'extension_period' => 'required|in:1,2,3,6,12,24,36',
        ]);

        $user = User::find($user_id);
        if (!$user) {
            \Log::error('User not found for subscription extension', ['user_id' => $user_id]);
            return response()->json(['error' => 'User not found'], 404);
        }

        \Log::info('User found for subscription extension', [
            'user_id' => $user->id,
            'user_name' => $user->name,
            'current_plan_expire_date' => $user->plan_expire_date
        ]);

        $extensionResult = $user->extendSubscription($request->extension_period);
        
        \Log::info('Extension result', $extensionResult);
        
        if ($extensionResult['is_success']) {
            // Send email notifications
            try {
                // Send email to user
                Mail::to($user->email)->send(new SubscriptionExtensionUser(
                    $user, 
                ...
                $admin = Auth::user();
                Mail::to('no-reply@meishicadi.com')->send(new SubscriptionExtensionAdmin(
                    $user,
                    $admin,
                    $request->extension_period,
                    \Auth::user()->dateFormat($user->plan_expire_date)
                ...
                // Log the email error but don't fail the extension
                \Log::error('Email sending failed for subscription extension: ' . $e->getMessage());
            }

            return response()->json([
                'success' => true,
                'message' => 'Subscription extended successfully.',
                'new_expire_date' => $user->plan_expire_date ? \Auth::user()->dateFormat($user->plan_expire_date) : 'Lifetime'
            ]);
        } else {
            \Log::error('Subscription extension failed', $extensionResult);
            return response()->json([
                'success' => false,
                'message' => $extensionResult['error'] ?? 'Failed to extend subscription.'
            ], 400);
        }
    }
