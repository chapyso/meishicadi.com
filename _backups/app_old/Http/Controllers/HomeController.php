            ...
            if (\Auth::user()->can('show dashboard')) {
                ...
                if (!file_exists(storage_path() . "/installed")) {
                    ...
                    if (\Auth::user()->type == 'super admin') {
                        ...
                        // Add pending expired plan requests count
                        $user['pending_expired_requests'] = \App\Models\plan_request::where('status', 'pending')->count();
                        $user['recent_expired_requests'] = \App\Models\plan_request::where('status', 'pending')
                            ->with(['user', 'plan'])
