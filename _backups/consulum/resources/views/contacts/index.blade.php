                ...
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <div class="form-group mb-0">
                            <select id="sort_by_country" class="form-select form-select-sm" onchange="sortByCountry()" style="min-width: 150px;">
                                <option value="none">{{ __('No Sort') }}</option>
                                <option value="country" {{ request('sort_by') == 'country' ? 'selected' : '' }}>{{ __('Sort by Country') }}</option>
                            </select>
                        </div>
                        <button class="csv btn btn-sm btn-primary export-btn mb-3">{{ __('Export to HubSpot') }}</button>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach ($contacts_deatails as $val)
                                <tr>
                                    ...
                                    <td>{{ $val->phone }}</td>
                                    <td>
                                        @if(!empty($val->country) && $val->country != 'Unknown')
                                            <span class="badge bg-info">{{ $val->country }} ({{ $val->phone_country_code }})</span>
                                        @else
...
@push('custom-scripts')
    ...
    <script>
        ...
        function sortByCountry() {
            const sortBy = document.getElementById('sort_by_country').value;
            const url = new URL(window.location.href);
            if (sortBy === 'country') {
                url.searchParams.set('sort_by', 'country');
            } else {
