...
<body class="tech-card-body">
    <div id="boxes">
        <div class="{{ \App\Models\Utility::themeOne()['theme21'][$business->theme_color]['theme_name'] ?? \App\Models\Utility::themeOne()['theme21']['color1-theme21']['theme_name'] ?? 'theme21-v1' }}"
            id="view_theme10">
                                        ...
                                        @if (!is_null($contactinfo_content) && !is_null($contactinfo))
                                            @foreach ($contactinfo_content as $key => $val)
                                                @foreach ($val as $key1 => $val1)
                                                    ...
                                                    @if ($key1 != 'id')
                                                        @php
                                                            $svg_file = @file_get_contents(public_path('custom/theme21/icon/' . $color . '/contact/' . strtolower($key1) . '.svg'));
                                                            if ($svg_file === false) {
                                                                $svg_file = '';
                                                            }
                                                            $find_string = '<svg';
                                                            $position = $svg_file ? strpos($svg_file, $find_string) : false;
                                                            
                                                            $svg_file_new = ($position !== false) ? substr($svg_file, $position) : '';
 ...
 <style>
  ...
  @if ($order_key == 'map_iframe' )
                                    <section id="map_iframe_preview" class="card-footer print map_iframe" style="background:white !important; padding: 1px 15px;@if(!$is_map_iframe_enabled)
                                        ...
                                        <h2 class="text-center mb-3 " style="margin: 10px 0px ">{{ __('Address') }}</h2>
