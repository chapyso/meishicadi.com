...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected $fillable = [
        ...
        'is_enable_login',
        'created_by',
        'allow_gmail_sync', // Gmail sync admin control
        'wallet_premium', // Wallet premium access
        'wallet_premium_expires_at', // Wallet premium expiration
    ];
    ...
    public function hasProColorWheel(){
        // Color wheel is now available to all users
        return true;
    }

    public function hasWalletPremium(){
        // Check if user has wallet premium access
        if (!$this->wallet_premium) {
            return false;
        }
        
        // Check if premium has expired
        if ($this->wallet_premium_expires_at && $this->wallet_premium_expires_at->isPast()) {
            return false;
        }
    ...
    public function hasBulkTransferAccess()
    {
        ...
        $plan = Plan::find($this->plan);
        return $plan && $plan->enable_bulk_transfer == 'on';
    }
    ...
    public function hasWalletAccess()
    {
        ...
        $plan = Plan::find($this->plan);
        return $plan && $plan->enable_wallet == 'on';
    }
    
    public function hasIntegrationsAccess()
    {
        if ($this->type == 'super admin') {
            return true;
        }
        ...
        $plan = Plan::find($this->plan);
        return $plan && $plan->enable_integrations == 'on';
    }
    ...
    public function hasAnalyticsAccess()
    {
        ...
        $plan = Plan::find($this->plan);
        return $plan && $plan->enable_analytics == 'on';
    }
