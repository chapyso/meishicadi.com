...
if (!function_exists('lazy_load_image')) {
    /**
     * Generate lazy loading image HTML
     */
    function lazy_load_image($src, $alt = '', $class = '', $placeholder = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7')
    {
        return sprintf(
...
if (!function_exists('preload_asset')) {
     ...
     */
    function preload_asset($href, $as = 'style', $crossorigin = false)
    {
        $crossoriginAttr = $crossorigin ? ' crossorigin' : '';
        return sprintf('<link rel="preload" href="%s" as="%s"%s>', $href, $as, $crossoriginAttr);
    }
}
...
if (!function_exists('inline_critical_css')) {
    ...
    function inline_critical_css()
    {
        ...
        return sprintf('<style>%s</style>', $criticalCSS);
    }
}

if (!function_exists('defer_js')) {
    /**
     * Generate deferred JavaScript loading
     */
    function defer_js($src, $attributes = [])
    {
        $attrs = '';
        ...
        return sprintf('<script src="%s" defer%s></script>', $src, $attrs);
    }
}
...
/**
 * Get current language name from session or default
 */
if (!function_exists('getCurrentLanguageName')) {
    function getCurrentLanguageName()
    {
        return session('language_name', config('app.locale', 'en'));
    }
}

/**
 * Get language full name by language code/key
 */
if (!function_exists('getLanguageByKey')) {
    function getLanguageByKey($key)
    {
        try {
            $language = \App\Models\Languages::where('code', $key)->first();
            return $language ? $language->fullName : $key;
        } catch (\Exception $e) {
            return $key;
        }
    }
...
/**
 * Check and set frontend language session
 */
if (!function_exists('checkFrontLanguageSession')) {
    function checkFrontLanguageSession()
    {
        if (!session()->has('language_name')) {
            session(['language_name' => config('app.locale', 'en')]);
        }
    }
}
...
if (!function_exists('getLogoUrl')) {
    function getLogoUrl()
    {
        try {
            ...
            return $logo ? asset('uploads/logo/' . $logo) : asset('assets/images/logo.png');
        } catch (\Exception $e) {
...
if (!function_exists('getAppName')) {
    ...
    {
        try {
            $appName = \DB::table('settings')->where('key', 'company_name')->value('value');
            return $appName ?: config('app.name', 'Laravel');
        } catch (\Exception $e) {
