...
class SubscriptionController extends AppBaseController
{
    ...
    /**
     * Handle wallet add-on payment selection
     */
    public function chooseWalletAddOnPayment()
    {
        $currentSubscription = getCurrentSubscription();
        
        // Check if user already has wallet integration
        if (checkFeature('wallet_integration')) {
            Flash::error('Wallet integration is already active for your account.');
            return Redirect::back();
        }
        ...
        $paymentType = request()->get('payment_type', 'stripe');

        // Create a virtual plan for wallet add-on
        $walletAddOnPlan = (object) [
            'id' => 'wallet-addon',
            'name' => 'Wallet Integration Add-On',
            'price' => 1.00,
            'currency' => (object) [
                'currency_code' => 'USD',
                'currency_icon' => '$'
            ],
            'description' => 'Add your vCard to Apple Wallet and Google Wallet',
        ...
        if ($paymentType === 'stripe') {
            return $this->handleStripeWalletPayment($walletAddOnPlan);
        } elseif ($paymentType === 'paypal') {
            return $this->handlePayPalWalletPayment($walletAddOnPlan);
        } elseif ($paymentType === 'paystack') {
            return $this->handlePaystackWalletPayment($walletAddOnPlan);
        } elseif ($paymentType === 'razorpay') {
            return $this->handleRazorpayWalletPayment($walletAddOnPlan);
        }

        return view('subscription.payment_for_wallet_addon', compact('walletAddOnPlan', 'paymentTypes'));
    }

    /**
     * Handle Stripe wallet payment
     */
    private function handleStripeWalletPayment($walletAddOnPlan)
    {
        try {
            $user = Auth::user();
            ...
            $transaction = Transaction::create([
                ...
                'meta' => [
                    'is_wallet_addon' => true,
                    'description' => 'Wallet Integration Add-On',
                    'payment_gateway' => 'stripe'
                ]
            ]);

            // Enable wallet integration
            $this->enableWalletIntegration($user);

            Flash::success('Wallet Integration Add-On has been successfully activated!');
            return redirect()->route('subscription.index');

        } catch (\Exception $e) {
            Log::error('Stripe Wallet Payment Error: ' . $e->getMessage());
    ...
    /**
     * Handle PayPal wallet payment
     */
    private function handlePayPalWalletPayment($walletAddOnPlan)
    {
        try {
            $user = Auth::user();
            ...
            Log::error('PayPal Wallet Payment Error: ' . $e->getMessage());
            Flash::error('Payment failed. Please try again.');
            return redirect()->back();
        }
    }
    ...
    /**
     * Handle Paystack wallet payment
     */
    private function handlePaystackWalletPayment($walletAddOnPlan)
    {
        try {
            $user = Auth::user();
            ...
            Log::error('Paystack Wallet Payment Error: ' . $e->getMessage());
            Flash::error('Payment failed. Please try again.');
            return redirect()->back();
        }
    }
    ...
    /**
     * Handle Razorpay wallet payment
     */
    private function handleRazorpayWalletPayment($walletAddOnPlan)
    {
        try {
            $user = Auth::user();
            ...
            Log::error('Razorpay Wallet Payment Error: ' . $e->getMessage());
            Flash::error('Payment failed. Please try again.');
            return redirect()->back();
        }
    }
    ...
    /**
     * Enable wallet integration for user
     */
    private function enableWalletIntegration($user)
    {
        $currentSubscription = getCurrentSubscription();
        if ($currentSubscription) {
            $planFeature = $currentSubscription->plan->planFeature;
            if ($planFeature) {
                $planFeature->update([
        ...
        $emailData = [
            'subscriptionId' => time(),
            'subscriptionAmount' => 1.00,
            'transactionID' => time(),
            'planName' => 'Wallet Integration Add-On',
    ...
    public function purchaseSubscription(Request $request)
    {
        ...
        // Handle wallet add-on purchase
        if (isset($input['is_wallet_addon']) && $input['is_wallet_addon']) {
            return $this->handleWalletAddOnPurchase($input);
        }
     ...
     * Handle wallet add-on purchase
     */
    private function handleWalletAddOnPurchase($input)
    {
        try {
            DB::beginTransaction();
            ...
            // Check if user already has wallet integration
            if (checkFeature('wallet_integration')) {
                return $this->sendError('Wallet integration is already active for your account.');
            }

            // Get payment method
            $paymentType = $input['payment_type'] ?? 'stripe';
            
            // Create a transaction record for the wallet add-on
            $transaction = Transaction::create([
                'user_id' => $user->id,
                'tenant_id' => $tenantId,
            ...
            // Enable wallet integration for the user's current plan
            $currentSubscription = getCurrentSubscription();
            if ($currentSubscription) {
                // Update the plan features to include wallet integration
                $planFeature = $currentSubscription->plan->planFeature;
                if ($planFeature) {
                    $planFeature->update([
                        'wallet_integration' => true
            ...
            return $this->sendSuccess('Wallet Integration Add-On has been successfully activated!');
            
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Wallet Add-On Purchase Error: ' . $e->getMessage());
            return $this->sendError('Failed to process wallet add-on purchase. Please try again.');
        }
    }
