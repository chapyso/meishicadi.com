...
Route::group(['middleware' => ['verified']], function () {
    ...
    Route::post('/appointment/update-status/{id}', [AppointmentDeatailController::class, 'updateStatus'])->middleware('XSS', 'auth')->name('appointment.update-status');
    Route::get('/appointment/statistics', [AppointmentDeatailController::class, 'getStatistics'])->middleware('XSS', 'auth')->name('appointment.statistics');
    Route::get('/appointment/available-slots', [AppointmentDeatailController::class, 'getAvailableSlots'])->middleware('XSS', 'auth')->name('appointment.available-slots');
...
//uncomment following line
    Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
        ...
        // Coupon routes - restricted to super admin only
        Route::middleware(['auth', 'XSS'])->group(function () {
            Route::get('applycoupon', [CouponController::class, 'applyCoupon'])->name('apply.coupon');
    ...
    Route::get('businessqr/download/', [BusinessController::class, 'downloadqr'])->middleware('XSS', 'auth')->name('download.qr');
        ...
        // Wallet Routes
Route::get('wallet', [WalletController::class, 'index'])->middleware('XSS', 'auth')->name('wallet.index');
Route::post('wallet/generate/apple/{businessId}', [WalletController::class, 'generateAppleWallet'])->middleware('XSS', 'auth')->name('wallet.generate.apple');
Route::post('wallet/generate/google/{businessId}', [WalletController::class, 'generateGoogleWallet'])->middleware('XSS', 'auth')->name('wallet.generate.google');
Route::get('wallet/download/apple/{passId}', [WalletController::class, 'downloadApplePass'])->middleware('XSS', 'auth')->name('wallet.download.apple');
Route::get('wallet/google/link/{passId}', [WalletController::class, 'getGoogleWalletLink'])->middleware('XSS', 'auth')->name('wallet.google.link');

// Wallet Subscription Routes
Route::get('wallet/check-premium', [App\Http\Controllers\WalletSubscriptionController::class, 'checkPremium'])->middleware('XSS', 'auth')->name('wallet.check-premium');
Route::post('wallet/subscribe', [App\Http\Controllers\WalletSubscriptionController::class, 'subscribe'])->middleware('XSS', 'auth')->name('wallet.subscribe');
   ...
   Route::any('/paytr/success', [PaytrController::class, 'paytrsuccess'])->name('pay.paytr.success');
    ...
    // Email template routes - restricted to super admin only
    Route::middleware(['auth', 'XSS'])->group(function () {
...
// Gmail OAuth routes for business (VCard) owners
Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
    Route::get('business/{id}/gmail/oauth', [App\Http\Controllers\BusinessController::class, 'startGmailOAuth'])->name('business.gmail.oauth');
    Route::get('business/{id}/gmail/callback', [App\Http\Controllers\BusinessController::class, 'handleGmailOAuthCallback'])->name('business.gmail.callback');
});

 Route::get('/home', [HomeController::class, 'index'])->middleware('XSS', 'auth', 'CheckPlan')->name('home.dashboard');
 Route::get('/home', [App\Http\Controllers\HomeController::class, 'index'])->middleware('XSS', 'auth', 'CheckPlan')->name('home');

// Tap tracking routes
Route::post('/tap/{slug}', [TapController::class, 'trackTap'])->name('tap.track');
Route::get('/tap/{slug}/count', [TapController::class, 'getTapCount'])->name('tap.count');
Route::get('/tap/statistics', [TapController::class, 'getTapStatistics'])->middleware('XSS', 'auth')->name('tap.statistics');

// Enhanced analytics routes
Route::middleware(['auth', 'XSS'])->group(function () {
    Route::get('/tap/business/{businessId}/analytics', [TapController::class, 'getBusinessAnalytics'])->name('tap.business.analytics');
    Route::get('/tap/business/{businessId}/logs', [TapController::class, 'getTapLogs'])->name('tap.business.logs');
    Route::get('/tap/business/{businessId}/export', [TapController::class, 'exportTapLogs'])->name('tap.business.export');
});

// Global Tap Analytics routes (Super Admin only)
Route::middleware(['auth', 'XSS'])->group(function () {
    Route::get('/tap-analytics', [TapAnalyticsController::class, 'index'])->name('tap-analytics.index');
    Route::get('/tap-analytics/statistics', [TapAnalyticsController::class, 'getGlobalStatistics'])->name('tap-analytics.statistics');
    Route::get('/tap-analytics/time-analytics', [TapAnalyticsController::class, 'getTimeAnalytics'])->name('tap-analytics.time-analytics');
    Route::get('/tap-analytics/recent-activity', [TapAnalyticsController::class, 'getRecentActivity'])->name('tap-analytics.recent-activity');
    Route::get('/tap-analytics/export', [TapAnalyticsController::class, 'export'])->name('tap-analytics.export');
});
...
Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
    ...
    Route::post('/integrations/webhook', [IntegrationController::class, 'storeWebhook'])->name('integrations.webhook.store');
    Route::put('/integrations/webhook/{id}', [IntegrationController::class, 'updateWebhook'])->name('integrations.webhook.update');
    Route::post('/integrations/webhook/{id}/test', [IntegrationController::class, 'testWebhook'])->name('integrations.webhook.test');
    Route::get('/integrations/webhook/{id}/logs', [IntegrationController::class, 'getWebhookLogs'])->name('integrations.webhook.logs');
    
    // CRM integrations
    Route::post('/integrations/hubspot/connect', [IntegrationController::class, 'connectHubSpot'])->name('integrations.hubspot.connect');
    Route::post('/integrations/zoho/connect', [IntegrationController::class, 'connectZoho'])->name('integrations.zoho.connect');
    Route::post('/integrations/softchap/connect', [IntegrationController::class, 'connectSoftchap'])->name('integrations.softchap.connect');
    
    // General integration management
    Route::delete('/integrations/{id}', [IntegrationController::class, 'destroy'])->name('integrations.destroy');
    Route::post('/integrations/{id}/toggle', [IntegrationController::class, 'toggleStatus'])->name('integrations.toggle');
    Route::post('/integrations/{id}/disconnect', [IntegrationController::class, 'disconnectCrm'])->name('integrations.disconnect');
    
    // Admin statistics
    Route::get('/integrations/statistics', [IntegrationController::class, 'getStatistics'])->name('integrations.statistics');
});

// OAuth callback routes (no auth middleware)
Route::get('/integrations/hubspot/callback', [IntegrationController::class, 'hubSpotCallback'])->name('integrations.hubspot.callback');
Route::get('/integrations/zoho/callback', [IntegrationController::class, 'zohoCallback'])->name('integrations.zoho.callback');

// Wallet routes
Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
    Route::get('/wallet', [WalletController::class, 'index'])->name('wallet.index');
    Route::post('/wallet/generate/apple/{businessId}', [WalletController::class, 'generateAppleWallet'])->name('wallet.generate.apple');
    Route::post('/wallet/generate/google/{businessId}', [WalletController::class, 'generateGoogleWallet'])->name('wallet.generate.google');
    Route::get('/wallet/download/apple/{passId}', [WalletController::class, 'downloadApplePass'])->name('wallet.download.apple');
    Route::get('/wallet/google/link/{passId}', [WalletController::class, 'getGoogleWalletLink'])->name('wallet.google.link');
});

// Business analytics routes
Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
    Route::get('/business/{id}/analytics', [BusinessController::class, 'analytics'])->name('business.analytics');
});

Route::get('/all-businesses', [BusinessController::class, 'showAllBusinesses']);
...
Route::middleware(['auth', 'XSS'])->group(function () {
    ...
    Route::post('/bulk-transfer', [BulkTransferController::class, 'store'])->name('bulk-transfer.store');
    Route::delete('/bulk-transfer/{id}', [BulkTransferController::class, 'destroy'])->name('bulk-transfer.destroy');
    Route::get('/bulk-transfer/stats', [BulkTransferController::class, 'getStats'])->name('bulk-transfer.stats');
});

// Public download route (no auth required)
Route::get('/bulk-transfer/download/{token}', [BulkTransferController::class, 'download'])->name('bulk-transfer.download');

// Admin bulk transfer routes (super admin only)
Route::middleware(['auth', 'XSS'])->group(function () {
    Route::get('/admin/bulk-transfer', [BulkTransferController::class, 'adminIndex'])->name('bulk-transfer.admin.index');
    Route::get('/admin/bulk-transfer/settings', [BulkTransferController::class, 'adminSettings'])->name('bulk-transfer.admin.settings');
    Route::post('/admin/bulk-transfer/settings', [BulkTransferController::class, 'updateGlobalSettings'])->name('bulk-transfer.admin.settings.update');
    Route::post('/admin/bulk-transfer/delete-expired', [BulkTransferController::class, 'bulkDeleteExpired'])->name('bulk-transfer.admin.delete-expired');
});

// Temporary test route for debugging modern color picker
Route::get('/test-color-picker', function() {
    $user = \App\Models\User::where('email', 'Info@chapysocial.com')->first();
    if ($user) {
        \Auth::login($user);
        return view('test-color-picker');
    }
