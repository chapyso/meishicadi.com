...
//uncomment following line
    Route::middleware(['auth', 'XSS', 'CheckPlan'])->group(function () {
        ...
        Route::delete('users/bulk', [UserController::class, 'bulkDestroy'])->name('users.bulk.destroy')->middleware(['XSS', 'auth']);
...
// Subdomain route - handle root path on subdomain
// This will match any subdomain (e.g., subdomain.example.com)
// Note: You may need to adjust the domain pattern based on your main domain
Route::domain('{subdomain}')->group(function () {
    Route::get('/', [BusinessController::class, 'getcardBySubdomain'])->name('subdomain.card');
    Route::get('/download', [BusinessController::class, 'getVcardDownloadBySubdomain'])->name('subdomain.download');
    // Note: You may need to add storeBySubdomain methods to other controllers
    // For now, these routes will fall back to regular routes if subdomain doesn't match
});

// Regular slug-based routes
