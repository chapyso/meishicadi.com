...
class HomeController extends Controller
{
            ...
            if (\Auth::user()->can('show dashboard')) {
                ...
                if (!file_exists(storage_path() . "/installed")) {
                    ...
                    if (\Auth::user()->type == 'super admin') {
                        ...
                        $quickBusinesses = Business::where('created_by', \Auth::user()->creatorId())
                            ->with('contactInfo')
                            ->orderBy('id', 'desc')
                            ->take(10)
                            ->get();

                        foreach ($quickBusinesses as $quickBusiness) {
                            $countryInfo = get_business_country_from_contactinfo($quickBusiness->contactInfo);
                            $quickBusiness->country = $countryInfo['country'];
                            $quickBusiness->country_code = $countryInfo['code'];
                            $quickBusiness->phone_country_code = $countryInfo['country_code'];
                        }

                        return view('dashboard.dashboard', compact('total_bussiness', 'total_app', 'visitor_url', 'devicearray', 'browserarray', 'cards', 'users', 'plan', 'storage_limit', 'businessData', 'qr_detail', 'total_staff', 'quickBusinesses'));
                    }
    ...
    public function getOrderChart($arrParam)
    {
        ...
        foreach ($business as $b) {
            ...
            // Get the most recent appointment for this business to find the appointment user
            $recentAppointment = \DB::table('appointment_deatails')
                ->where('business_id', $b->id)
                ->where('created_by', \Auth::user()->creatorId())
                ->orderBy('created_at', 'DESC')
                ->first();
            
            // Try to get the appointment user's avatar instead of business owner's
            if ($recentAppointment && !empty($recentAppointment->email)) {
                // Try to find a User by email (the appointment maker)
                $appointmentUser = User::where('email', $recentAppointment->email)->first();
                if ($appointmentUser && !empty($appointmentUser->avatar)) {
                    $d['avatar'] = $profile . '/' . $appointmentUser->avatar;
                } else {
                    // Fallback: if appointment user doesn't have account, use business owner
                    $businessOwner = User::find($b->created_by);
                    if ($businessOwner && !empty($businessOwner->avatar)) {
                        $d['avatar'] = $profile . '/' . $businessOwner->avatar;
                    } else {
                        $d['avatar'] = $profile . '/avatar.png';
                    }
                }
            } else {
                // No appointments yet, use business owner avatar
                $businessOwner = User::find($b->created_by);
                if ($businessOwner && !empty($businessOwner->avatar)) {
                    $d['avatar'] = $profile . '/' . $businessOwner->avatar;
                } else {
