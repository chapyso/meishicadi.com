  ...
  @if ($order_key == 'map_iframe' )
    ...
    <script>
        ...
        function downloadVCard() {
            var businessName = {!! json_encode($business->title ?? "") !!};
            var businessDesignation = {!! json_encode($business->designation ?? "") !!};
            var businessSubTitle = {!! json_encode($business->sub_title ?? "") !!};
            ...
            $(".contact-value.Phone_preview, .contact-value.Telephone_preview").each(function() {
            ...
            $(".contact-value.Email_preview").each(function() {
                var emailText = $(this).text().trim();
                if (emailText && emailText.includes("@") && !email) {
                    email = emailText;
                }
            });
            
            $(".contact-value.Address_preview").each(function() {
                var addrText = $(this).text().trim();
                if (addrText && !address) {
                    address = addrText;
                }
            });
            
            $(".contact-value.Web_url_preview").each(function() {
                var webText = $(this).text().trim();
                if (webText && !website) {
                    website = webText;
                }
            });
            ...
            vcard += "VERSION:3.0\n";
            vcard += "FN:" + businessName + "\n";
            vcard += "N:" + businessName + ";;;\n";
            
            if (businessDesignation) {
                vcard += "TITLE:" + businessDesignation + "\n";
            }
            
            if (businessSubTitle) {
                vcard += "ORG:" + businessSubTitle + "\n";
            }
            ...
            link.href = url;
            link.download = businessName.replace(/[^a-z0-9]/gi, "_").toLowerCase() + ".vcf";
